...
public class MyScheduledTask2 {
    ...
    public void scheduledTask2(){
        List<MedicalAdviceQueryVO> ma = medicalAdviceMapper.selectDrugApply();
        List<DrugApplicationVO> drugApplicationVOS = DrugConverter.INSTANCE.ListDrugApplication(ma);
        int insert = 0;
        if (drugApplicationVOS.size()>0) {
            insert = insert + drugApplicationMapper.insert(drugApplicationVOS);
        }

        SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");//设置日期格式
        SysLog sysLog=new SysLog();
        sysLog.setUri("定时任务2");//请求接口
        sysLog.setMethod("定时任务2");//请求方式
        sysLog.setMethodDescribe("定时任务2");//描述
        sysLog.setParams("更改了"+String.valueOf(insert));//参数信息
        sysLog.setUsername("定时任务2");//请求人
        sysLog.setIp("定时任务2");//ip
